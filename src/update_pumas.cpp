...
void update_pumas(board<tile> &field, double timeStep, double b, double l, double m, double r)
{
	...
	{
		new_field(x,y).puma = field(x,y).puma + timeStep* (b*field(x,y).hare*field(x,y).puma - m*field(x,y).puma + l*(field(x-1,y).puma + field(x+1,y).puma + field(x,y-1).puma + field(x,y+1).puma) - (field(x-1,y).is_land() + field(x+1,y).is_land() + field(x,y-1).is_land() + field(x,y+1).is_land())*field(x,y).puma);
	}
